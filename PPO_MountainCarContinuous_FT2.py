...
def test_reward():
    ...
    while not done:
        ...
        action = model_actor.predict([state_imput, dummy_n, dummy_1, dummy_1, dummy_1], steps = 1).reshape(1,)
...
def get_model_actor(input_dims, output_dims):
    ...
    x = kl.Dense(256, kernel_initializer='random_uniform', activation='tanh', name='fc0')(state_input)
...
if __name__ == '__main__':
    ...
    while not target_reached and iters < max_iters:
        ...
        for itr in range(ppo_steps):
            ...
            if done:
                print('ACHIEVED!!!!!!!!!!!!')
                print('Ep: '+ str(iters) + ' succ: ' + str(success) + ' itr: ' + str(itr) + ', action=' + str(action) + ', reward=' + str(reward) + ', q val=' + str(q_value))
                success += 1
                env.reset()
        
        print('Ep: '+ str(iters) + ' succ: ' + str(success) + ', reward=' + str(sum(rewards)))
        ...
        # input('Waiting for check')
        oldpolicy_probs, advantages, rewards, values = np_actions_probs, np_advantages, np_rewards, np_values
        model_actor.fit([np_states, np_actions_probs, np_advantages, np_rewards, np_values],[np_actions_deter],verbose=True, shuffle=True, epochs=8)
        model_critic.fit([np_states], [np_returns],verbose=True, shuffle=True, epochs=8)
        # input('Waiting for check')
    ...
    model_actor.save('PPO_Car_model_actor.h5')
    model_critic.save('PPO_Car_model_critic.h5')
    env.close()
